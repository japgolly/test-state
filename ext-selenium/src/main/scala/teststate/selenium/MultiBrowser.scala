...
object MultiBrowser {
  def apply[D <: WebDriver: TabSupport](newDriver: => D, growthStrategy: GrowthStrategy): MultiBrowser[D] =
    new MultiBrowser[D](() => newDriver, growthStrategy)
}

final class MultiBrowser[D <: WebDriver](newDriver: () => D,
                                         growthStrategy: GrowthStrategy)
                                        (implicit tabSupport: TabSupport[D]) {
  ...
  // Locks: outer(browser)
  def closeBrowser(i: Int, quit: Boolean = true): Unit =
    outerMutex {
