...
object Simplify {
  ...
  final class DomCollection[Z[f[_]] <: DomZipper2[f, A], F[_], C[_], A](
       from      : Z[F],
  ...
  object DomCollection {
    ...
    final class Container01[F[_]](implicit h: ErrorHandler[F]) extends Container[F, Option] {
    ...
    final class Container0N[F[_]](implicit h: ErrorHandler[F]) extends Container[F, Vector] {
      override def apply[A](desc: String, es: CssSelResult[A]) =
        h pass es
    ...
    final class Container1N[F[_]](implicit h: ErrorHandler[F]) extends Container[F, Vector] {
      override def apply[A](desc: String, es: CssSelResult[A]) =
        if (es.isEmpty)
